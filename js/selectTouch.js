!function ($) {
    ...
    var selectTouch = function (options) {
        ...
        var selectTouchOptions = {
            selectTouchWarp: '.gxs-selectTouchs',
        ...
        this.prototype = {
            nextPage: function () {
                ...
                selectTouchOptions.item = Math.abs(Math.floor(parseFloat($(selectTouchOptions.selectTouchSlide).css("top")) / 49));
            ...
            objAnimation: function () {
                var setEq = -(selectTouchOptions.item * 49);
        ...
        var date = new Date();
        selectTouchOptions.dialogWarp = '.gxs-selectTouchs-' + date.getTime();
        ...
        $(selectTouchOptions.selectTouchSlide).on('touchmove', function (event) {
            ...
            if (selectTouchOptions.isScrolling) {
                var moveOffset = movePos.y - selectTouchOptions.item * 49;
        ...
        $(selectTouchOptions.selectTouchSlide).on('touchend', function (event) {
            ...
            if (duration > 0) { /*间隔事件*/
                if (Math.abs(endPos.y) > 10) { /*两次滑动的距离>10*/
                    if (endPos.y >= 0) {
                        if (selectTouchOptions.item == 0) {
